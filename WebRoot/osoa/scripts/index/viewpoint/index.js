 ...
 * 投研观点列表
 */
define("osoa/scripts/index/viewpoint/index", function(require, exports, module) {
    var pageId = "#index_viewpoint_index ";
    ...
    function initArticleList(page, pageSize) {
                ...
                if (results && results[0].data.length > 0) {
                    ...
                    for (var i = 0; i < results[0].data.length; i++) {
                        ...
                        if (results[0].data[i].type != "0") {
                            ...
                            str += '<a target="_blank" href="/osoa/views/article/' + results[0].data[i].article_id + '.html"><span>' + results[0].data[i].publish_date + '</span><strong>' + results[0].data[i].title + '</strong></a>';
                        }
        ...
        }, { page: page, pageSize: pageSize, catalog_id: '10281' })
    }
    ...
    // 暴露对外的接口
    module.exports = index_viewpoint_index;
});