 ...
 * 专户知识  --专户理财
 */
define("osoa/scripts/separate/laws/index", function(require, exports, module) {
    var layerUtils = require("layerUtils");
    var pageId = "#separate_laws_index ";
    ...
    function initArticleList(page, pageSize) {
            ...
            if (error_no == 0) {
                ...
                if (results && results[0].data.length > 0) {
                    ...
                    for (var i = 0; i < results[0].data.length; i++) {
                        ...
                        // str += '<a target="_blank" href="/osoa/views/separate/laws/detail/' + results[0].data[i].article_id + '.html"><span>' + results[0].data[i].publish_date + '</span><strong>' + results[0].data[i].title + '</strong></a>';
                        if (results[0].data[i].type != "0") {
                            ...
                            str += '<a target="_blank" href="/osoa/views/separate/laws/detail/detail.html?article_id=' + results[0].data[i].article_id + '"><span>' + results[0].data[i].publish_date + '</span><strong>' + results[0].data[i].title + '</strong></a>';
                        }
        ...
        }, { page: page, pageSize: pageSize, catalog_id: '10320', fund_code: fund_code })
    }
    ...
    var separate_laws_index = {
        "init": init,
        "bindPageEvent": bindPageEvent,
        "destroy": destroy
    };
    // 暴露对外的接口
    module.exports = separate_laws_index;
});