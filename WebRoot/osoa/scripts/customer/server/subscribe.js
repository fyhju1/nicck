 ...
 * 关于华商 ---公司简介
 */
define("osoa/scripts/customer/server/subscribe", function(require, exports, module) {
    var pageId = "#customer_server_subscribe ";
    ...
    function initArticleList(page, pageSize) {
                ...
                if (results && results[0].data.length > 0) {
                    ...
                    for (var i = 0; i < results[0].data.length; i++) {
                        ...
                        if (results[0].data[i].type != "0") {
                            ...
                            str += '<a target="_blank" href="/osoa/views/customer/server/subscribe/detail.html?article_id=' + results[0].data[i].article_id + '"><span>' + results[0].data[i].publish_date + '</span><strong>' + results[0].data[i].title + '</strong></a>';
                        }
        ...
        }, { page: page, pageSize: pageSize, catalog_id: '10285' })
    }
    ...
    var customer_server_subscribe = {
        "init": init,
        "bindPageEvent": bindPageEvent,
        "destroy": destroy
    };
    // 暴露对外的接口
    module.exports = customer_server_subscribe;
});