 ...
 * 赎回费率计算器
 */
define("osoa/scripts/index/calculator/redeemrate", function(require, exports, module) {
    ...
    function bindPageEvent() {
        ...
        $(pageId + " .core #calculator_submit").bindEvent(function() {
            ...
            if (!isNaN(red_quotient)) {
                if (red_quotient < 0) {
                    layerUtils.iAlert("赎回金额不能小于0");
                    return;
                }
            } else {
                layerUtils.iAlert("赎回金额为数字");
                return;
            }
            if (!isNaN(nav)) {
                if (nav < 0) {
            ...
            if (!isNaN(red_rate)) {
                if (red_rate > 100 || red_rate < 0) {
                    layerUtils.iAlert("赎回费率位于0~100之间");
                    return;
                }
            } else {
