...
RangeSelector.prototype = {
	...
	/**
	 * 添加确认按钮，add by duanxy 20140419
	 */
    drawButton:function(name){
		...
		//alert(name);
		this[name+"Input"]=input=createElement('a', {
			name: name
			...
			width: '40px', // Chrome needs a pixel to see it
			height: '18px',
			padding: 0,
			textAlign: 'center',
			fontSize: chartStyle.fontSize,
			fontFamily: chartStyle.fontFamily,
			top: chart.plotTop-26 + PX, // prevent jump on focus in Firefox
			left:680+PX,
		...
		input.aferClick=function(){
			//回调函数 执行完重新设置查询时间
			//alert(2222);
				
				setTimeout(this.setValue, 5000);
		};
		input.setValue=function(){
			extremes = chart.xAxis[0].getExtremes();
			//alert(dateFormat(options.inputEditDateFormat || '%Y-%m-%d', extremes.min))
			rangeSelector.setInputValue("min",extremes.min);
			rangeSelector.setInputValue("max",extremes.max);
			//alert('xyduane')
		}
		...
		input.onclick=function(aferClick){
			//alert($("input[name='max']").val());
			var startDate=$(this).parent().children("input[name='min']").val();//$("input[name='min']").val();
			//var startDate=$(this).prevAll().find("input[name='min']").val();
			
			//var startDate=( Date.parse)(inputStartDate),
			var endDate=$(this).parent().children("input[name='max']").val();//$("input[name='max']").val();
			//var endDate=(options.inputDateParser || Date.parse)(inputEndDate),
			if (isNaN(startDate)) {
					startDate = startDate.split('-');
					startDate = Date.UTC(pInt(startDate[0]), pInt(startDate[1]) - 1, pInt(startDate[2]));
			}
			if (isNaN(endDate)) {
					endDate = endDate.split('-');
					endDate = Date.UTC(pInt(endDate[0]), pInt(endDate[1]) - 1, pInt(endDate[2]));
			}
			...
			//this.aferClick();//回调函数
			//setTimeout(this.aferClick(), 5000000);
	...
	drawInput: function (name) {
		...
		inputGroup.offset += dateBox.width + (isMin ? 6 : 80);//xyduane modify
		...
		input.onblur = function () {
			...
			rangeSelector.setInputValue(name); //xyduane
			//alert(name)
		};
		...
		input.onchange = function () {
            ...
            rangeSelector.setInputValue(name,value);//xyduane
			if (!isNaN(value)) {
				 rangeSelector.setInputValue(name,value);//xyduane
	...
	render: function (min, max) {
		...
		if (!rangeSelector.rendered) {
			...
			if (inputEnabled) {
				...
				rangeSelector.drawButton('OK');//add by duanxy 20140419
			}
		}
		...
		if (inputEnabled) {
			...
			rangeSelector.setInputValue('min', min);   //xyduane
			rangeSelector.setInputValue('max', max);  //xyduane
		}
